...
| 포트 | 8089-8096 (8xxx 규칙). 실험 레플리카는 8003 |
...
측정 중에는 컨테이너를 내리지 않는다. Web UI 에서 결과를 계속 볼 수 있고, 수동으로
재실행해도 `monitor.sh` 가 대기 상태로 남아 가드가 걸린다.

**측정이 끝나면 `export.sh` 로 데이터를 내보낸 뒤 컨테이너를 정리한다.** UI 통계는
컨테이너 메모리에만 있으므로 순서를 지켜야 한다. 정리하지 않으면 포트와 RAM 상한이
계속 몫에서 빠져나가고, 실험 레플리카를 띄울 여유가 없어진다(실측: 부하기 8개가
RAM 상한 합에서 48g 를 차지했다).

```bash
./export.sh                                        # 먼저 내보내기
docker rm -f $(docker ps -aq --filter name=^vlm-stress-)   # 그 다음 정리
../scripts/exp-replica.sh down 8003                # 실험 레플리카가 있으면 같이
```

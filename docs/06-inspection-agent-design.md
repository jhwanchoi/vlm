...
1. **1차 (전 프레임)**: defect 클래스별 필드를 가진 단일 JSON을 뽑는 섹션형 루브릭 프롬프트 1회 호출.
   샘플링은 extraction 프리셋: **temperature 0-0.2, top_p 1.0** (판정/OCR 표준값)
2. **2차 (애매 판정 프레임만)**: 클래스별 분해 질문 + self-consistency 투표
   (N=3-5, temp 0.6-0.8, 다수결). 1차와 temperature가 다른 것은 의도: 투표는 다양성이 필요

에스컬레이션(2차행) 라우팅 기준 후보 2가지, 평가셋에서 비교:

- 스키마의 `confidence` 필드 (모델 자기보고, 캘리브레이션 불명)
- **판정 토큰(`verdict` enum)의 logprobs** (API `logprobs` 파라미터.
  자기보고보다 캘리브레이션 신뢰 가능. 임계값은 평가셋에서 P/R 곡선으로)
...
- 회귀 비교를 위해 temperature/seed 고정 (judge의 run 간 자기 불일치는 문서화된 현상).
  단 seed 고정은 재현성 보장이 아니다: 엔진 버전/커널 변경 시 같은 seed도 출력이 달라지므로
  버전 경계를 넘는 비교는 반복 샘플 통계로
